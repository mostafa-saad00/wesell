...
class ShippingTeamOrderIndex extends Component
{
    ...
    public function render()
    {
        ...
        $shipping_team = ModeratorTeam::where('team_leader_id', $current_team_leader->id)->where('task_name', 'shipping')->get();
        ...
        $orders = Order::join('admin_order', 'orders.id', '=', 'admin_order.order_id')
                        ...
                        ->where('task_name', 'shipping')
                        ->orderBy('created_at')
                        ->paginate(15);


       
        return view('livewire.moderators.teamleaders.shipping-team-order-index', ['orders' => $orders, 'order_statuses' => $order_statuses, 'shipping_team' => $shipping_team]);
    }
}
